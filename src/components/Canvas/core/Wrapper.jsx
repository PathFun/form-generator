  ...
  const originNode = (
    ...
    >
      ...
      <div className="absolute top-0 right-1 f7">
        {!inside && $id !== '#' && !hideId && <span className={hasDuplicateId ? 'red' : 'blue'}>{shownId}</span>}
      ...
      {!inside && $id !== '#' && isSelected && _numOfBtns > 0 && (
        <div className="pointer-wrapper">
          ...
          {_extraBtns.map((item, idx) => {
            return (
              <div key={idx.toString()} className="pointer" onClick={e => item.onClick && item.onClick(e, schema)}>
